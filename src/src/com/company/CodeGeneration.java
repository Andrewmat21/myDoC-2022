...
public class CodeGeneration {
        ...
        if (n.children.size() > 0) {
            ...
            switch (n.name) {
                ...
                case "AssignmentStatement":
                    ...
                    // codeGen for string assign
                        ...
                        //store string in heap
                            ...
                            System.out.println("DEBUG CodeGen - Writing [" + Integer.toHexString(heap+2).toUpperCase() + "] into memory");
                            code[position] = Integer.toHexString(heap+2).toUpperCase();
                            position++;
                        }
                    }
                ...
                case "IfStatement":
                    ...
                    String jumpTEMPEnd = "J" + jump;
                    jump++;
                    int ifStart = position;

                    System.out.println("DEBUG CodeGen - Writing [D0] into memory");
                    code[position] = "D0";
                    position++;

                    System.out.println("DEBUG CodeGen - Writing [" + jumpTEMPEnd + "] into memory");
                    code[position] = jumpTEMPEnd;
                    position++;

                    this.generate(n.children.get(1), progNum);

                    String ifJval = Integer.toHexString(position - (ifStart + 2)).toUpperCase();
    ...
    public void backpatch() {
            ...
            for (int j = 0; j < code.length; j++){
                if (staticData.get(i).t.equals(code[j])){
                    if (staticData.get(i).t2.equals(code[j+1])) {
                        ...
                        code[j + 1] = "00";
                        //code[j + 1] = Integer.toHexString().toUpperCase();
                    }
                }
            }
        }
        ...
        for (int n = 0; n < jumpTable.size(); n++){
            ...
            if (jumpTable.get(n).rep.length() < 2)
                jumpTable.get(n).rep = "0" + jumpTable.get(n).rep;
        }
    ...
    public void genEq(CSTNode n, int currentScope) {
        ...
        if (isDigit(n.children.get(0).name)){
            ...
            st1 = checkStatic(staticData, n.children.get(0).name, currentScope, 1);
            code[position] = st1;
            position++;

            st2 = checkStatic(staticData, n.children.get(0).name, currentScope, 2);
            code[position] = st2;
            position++;
        }
            ...
            else if (n.children.get(0).name.equals("false")){
                System.out.println("DEBUG CodeGen - Writing [F5] into memory");
                code[position] = "F5";
            }
            position++;
